...
#on fixe un seuil s=0,22000000. Si la moyenne est supérieure à ce seuil, on considère que les capteurs ne sont pas considérés comme étant similaires par rapport à cette variable
...
def verdict(m): #ce programme prend en argument la moyenne des distances normées entre deux capteurs pour une donnée précise 
    s=float(0.22000000) #on définit un seuil 
    if m>s:
